...
type DecodeMappingSet() =
    ...
    static member DecodeMappingSet (source: string) =
        ...
        let parseUri rawField = 
            match rawField with
            | Some text -> Some (NonRelativeURI.create text)
            | None -> None
        ...
        let parseEntityTypeEnum enum =
            match enum with
            | Some x -> Some (EntityTypeEnum.create x)
            | None -> None

        let parseDate date =
            match date with
            | Some d -> Some (Date.create d)
            | None -> None

        let mappingSetDecoder = 
            Decode.object (fun get ->
            MappingSet(
                ?Sssom_version = DecodeSssomVersion.Decode yamlElement,
                ?Curie_map = DecodeCurieMap.Decode yamlElement,
                ?Mapping_set_id = parseUri (get.Optional.Field "mapping_set_id" Decode.string),
                ?Mapping_set_version = get.Optional.Field "mapping_set_version" Decode.string,
                ?Mapping_set_source = parseUri (get.Optional.Field "mapping_set_source" Decode.string),
                ...
                ?Creator_id = parseEntityReference (get.Optional.Field "creator_id" Decode.string),
                ?Creator_label = get.Optional.Field "creator_label" Decode.string,
                ?License = parseUri (get.Optional.Field "license" Decode.string),
                ?Subject_type = parseEntityTypeEnum (get.Optional.Field "subject_type" Decode.string),
                ?Subject_source = parseEntityReference (get.Optional.Field "subject_source" Decode.string),
                ?Subject_source_version = get.Optional.Field "subject_source_version" Decode.string,
                ?Object_type = parseEntityTypeEnum (get.Optional.Field "object_type" Decode.string),
                ?Object_source = parseEntityReference (get.Optional.Field "object_source" Decode.string),
                ?Object_source_version = get.Optional.Field "object_source_version" Decode.string,
                ?Predicate_type = parseEntityTypeEnum (get.Optional.Field "predicate_type" Decode.string),
                ?Mapping_provider = parseUri (get.Optional.Field "mapping_provider" Decode.string),
                ...
                ?Mapping_tool_id = parseEntityReference (get.Optional.Field "mapping_tool_id" Decode.string),
                ?Mapping_tool_version = get.Optional.Field "mapping_tool_version" Decode.string,
                ?Mapping_date = parseDate (get.Optional.Field "mapping_date" Decode.string),
                ?Publication_date = parseDate (get.Optional.Field "publication_date" Decode.string),
                ?Subject_match_field = parseEntityReference (get.Optional.Field "subject_match_field" Decode.string),
                ?Object_match_field = parseEntityReference (get.Optional.Field "object_match_field" Decode.string),
                ?Subject_preprocessing = parseEntityReference (get.Optional.Field "subject_preprocessing" Decode.string),
                ?Object_preprocessing = parseEntityReference (get.Optional.Field "object_preprocessing" Decode.string),
                ?Similarity_measure = get.Optional.Field "similarity_measure" Decode.string,
                ?Curation_rule = parseEntityReference (get.Optional.Field "curation_rule" Decode.string),
                ?Curation_rule_text = get.Optional.Field "curation_rule_text" Decode.string,
                ?See_also = parseUri (get.Optional.Field "see_also" Decode.string),
                ?Issue_tracker = parseUri (get.Optional.Field "issue_tracker" Decode.string),
                ?Other = get.Optional.Field "other" Decode.string,
                ?Comment = get.Optional.Field "comment" Decode.string,
                ?Extension_definitions = get.Optional.Field "extension_definitions" (Decode.array DecodeExtensionDefinition.Decode)
            )
        )
