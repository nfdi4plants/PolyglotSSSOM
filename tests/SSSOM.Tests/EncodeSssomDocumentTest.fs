...
let tests = 
    testList "EncodeSssomDocument Tests" [

        testCase "should give valid sssom.tsv file from valid mapping & mappingSet objects" <| fun _ ->
            ...
            Expect.equal actual expected "Should correctly encode a full SssomDocument into a TSV string with YAML metadata"
    ]