...
let tests = 
    testList "DecodeSssomDocument Tests" [

        testCase "should return valid SssomDocument object for valid sssom.tsv file" <| fun _ ->
            let inputString =
                "#curie_map:\n" +
            ...
            Expect.equal reEncodeString inputString "Decoding and re-encoding a valid SSSOM document should yield the exact same string"
    ]